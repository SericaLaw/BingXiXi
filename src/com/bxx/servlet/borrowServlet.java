...
public class borrowServlet extends HttpServlet {
	 ...
	 *           accountName, "password": pwd } bvoInfo { SellerID:xx tel:xx
	 *           email:xx SellerName:xxx } storeInfo { StoreName:XX MarketPlaceID:xx
	 *           SellerID:xx MWS:xx } dropship{ push:1 } addtoWishlist{ list:1 }
	 *           bvo-orderPayment{ QTY RcverZip RcverTel RcverName RcvAddr
	 *           walletEmail total }
	 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case "/bvo-myInfo":
			...
			/*
			 * System.out.println(email); System.out.println(SellerName);
			 * System.out.println(SellerID); System.out.println(tel);
			 */
		...
		case "/bvo-shopAdd":// 添加店铺信息
			System.out.println(obj.size());
			System.out.println(obj);
			String StoreName = obj.get("StoreName").toString(), MarketPlaceID = obj.get("MarketPlaceID").toString(),
					SellerID_store = obj.get("SellerID").toString(), MWS = obj.get("MWS").toString();
		...
		case "/bvo-orderPayment":// 判断余额是否够，然后再添加信息
			System.out.println(obj);
			/*
			 * String QTY = obj.get("QTY").toString(), RcverZip =
			 * obj.get("RcverZip").toString(), RcverTel = obj.get("RcverTel").toString(),
			 * RcverName= obj.get(" RcverName").toString(), RcvAddr =
			 * obj.get("RcvAddr").toString();
			 */

		case "/walletpay":// 传totalnumber和钱包email
			//前端再传一个ordernumber
			Integer QTY = Integer.getInteger(obj.get("QTY").toString());
			String RcverZip = obj.get("RcverZip").toString(), RcverTel = obj.get("RcverTel").toString(),
					RcverName = obj.get("RcverName").toString(), RcvAddr = obj.get("RcvAddr").toString(),
					orderNumber=obj.get("orderNumer").toString();
			...
			if (isafford) {
				boolean confirm5 = FuncBorrow.add_payment_information(orderNumber, QTY, RcverZip, RcverTel, RcverName, RcvAddr);
			}
			obj.fluentPut("isafford", isafford);
			out.append(obj.toString());
			out.flush();
			break;
		case"/order_awaiting_shiped"://订单管理其余四个表格
			order_result=FuncBorrow.request_order_list("order_awaiting_shiped");
			out.append(JSON.toJSONString(order_result));
			out.flush();
			out.close();
			break;
		case"/order_shiped":
			order_result=FuncBorrow.request_order_list("order_shiped");
			out.append(JSON.toJSONString(order_result));
			out.flush();
			out.close();
			break;
		case"/order_completed_orders":
			order_result=FuncBorrow.request_order_list("order_completed_orders");
			out.append(JSON.toJSONString(order_result));
			out.flush();
			out.close();
			break;
		case"/order_cancelled_orders":
			order_result=FuncBorrow.request_order_list("order_cancelled_orders");
			out.append(JSON.toJSONString(order_result));
			out.flush();
			out.close();
		break;
